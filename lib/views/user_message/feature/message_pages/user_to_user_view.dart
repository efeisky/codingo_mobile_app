...
class UserToUserMessageView extends StatefulWidget {
  const UserToUserMessageView({Key? key}) : super(key: key);

  @override
  State<UserToUserMessageView> createState() => _UserToUserMessageViewState();
}

class _UserToUserMessageViewState extends State<UserToUserMessageView>
    with NavigatorMixin {
  String _talkedUsername = '';
  ...
  void initState() {
    ...
    _profileModel = UserProfileModel(username: '...', realname: '...', picture: '');
  ...
  Future<void> _fetchProfileData() async {
    final response = await _service.getUserProfileData(_talkedUsername);
    setState(() {
      _profileModel = response;
    });
  }

  Future<void> _fetchMessageData() async {
    final messages = await _service.getUserMessageData(_initialUsername, _talkedUsername);
    _messageStreamController.add(messages);
  }

  Future<void> _sendMessage(String message, String senderName, String receiverName) async {
    final response =
        await _service.sendMessage(message, senderName, receiverName, MessageMode.personel);
    if (response == true) {
      await _fetchMessageData();
    } else {
